...
bool LibinputHelper::isDeviceOfType(libinput_device* device, const QString& type)
{
    if (!device)
        return false;
    ...
    // Check based on type
    if (type.toLower() == "joystick") {
        const char* id_joystick = udev_device_get_property_value(udev_device, "ID_INPUT_JOYSTICK");
        if (id_joystick && strcmp(id_joystick, "1") == 0) {
            result = true;
        }
    } 
    else if (type.toLower() == "keyboard") {
        const char* id_keyboard = udev_device_get_property_value(udev_device, "ID_INPUT_KEYBOARD");
        if (id_keyboard && strcmp(id_keyboard, "1") == 0) {
            result = true;
        }
    }
    else if (type.toLower() == "mouse") {
        const char* id_mouse = udev_device_get_property_value(udev_device, "ID_INPUT_MOUSE");
        if (id_mouse && strcmp(id_mouse, "1") == 0) {
            result = true;
        }
    }
    else if (type.toLower() == "tablet") {
        const char* id_tablet = udev_device_get_property_value(udev_device, "ID_INPUT_TABLET");
        if (id_tablet && strcmp(id_tablet, "1") == 0) {
            result = true;
        }
    }
    else {
...
LibinputHelper::DeviceInfo LibinputHelper::getDeviceInfo(libinput_device* device)
{
    ...
    if (udev_device) {
        ...
        const char* devnode_str = udev_device_get_devnode(udev_device);
        if (devnode_str) {
            int fd = open(devnode_str, O_RDONLY | O_NONBLOCK);
            if (fd >= 0) {
                unsigned long evbit[NLONGS(EV_CNT)] = { 0 };
                unsigned long keybit[NLONGS(KEY_CNT)] = { 0 };
                unsigned long absbit[NLONGS(ABS_CNT)] = { 0 };
    ...
    // Mark as a joystick if it meets the criteria
    info.isJoystick = isJoystickDevice(device);
...
{
    // Just a more specific version of findInputDevices
    return findInputDevices("joystick");
}

QVector<LibinputHelper::DeviceInfo> LibinputHelper::findInputDevices(const QString& type)
{
    ...
    // Make a udev query for all input devices to find matching ones
    struct udev_enumerate* enumerate = udev_enumerate_new(m_udev);
    udev_enumerate_add_match_subsystem(enumerate, "input");
    
    // Add property match if looking for a specific type
    if (type.toLower() == "joystick") {
    ...
    udev_list_entry_foreach(entry, devices_list) {
        ...
        if (devnode) {
            ...
            if (libinput_dev) {
                // Check if it's the type of device we're looking for
                if (type.isEmpty() || isDeviceOfType(libinput_dev, type)) {
